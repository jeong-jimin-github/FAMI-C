...
class RunPage extends Component {
  ...
  render() {
    return (
      <div className="overflow-hidden">
        ...
        {this.state.running && !this.state.error ? (
          <TouchControls
            onPress={this.handleTouchPress}
            onRelease={this.handleTouchRelease}
          />
        ) : null}
      </div>
    );
  ...
  layout = () => {
    ...
    const touchControlsHeight = window.matchMedia?.("(max-width: 767px)").matches
      ? 210
      : 0;
    this.screenContainer.style.height = `${
      window.innerHeight - navbarHeight - touchControlsHeight
    }px`;
    if (this.emulator) {
  ...
  handleTouchPress = (button) => {
    if (this.emulator) this.emulator.pressButton(button);
  };

  handleTouchRelease = (button) => {
    if (this.emulator) this.emulator.releaseButton(button);
  };
